...
- **Segment-level granularity (TRA-001, fixed in round 5 Phase 8):** `TRANSLATE_SEGMENT` now operates per-leaf-segment (HEADING, PARAGRAPH, LIST_ITEM, TABLE_CELL) via `StructuralMap.iter_leaf_segments()`. The kernel walks the structural map, translates each leaf individually, and re-assembles via text replacement. This gives per-segment cache keys and per-segment evidence chains. Code-block (fenced + inline) no-translate zone protection is preserved via per-leaf inline-code stashing. When an LLM callback is supplied, whole-doc translation is used instead (LLMs typically translate whole documents).
...
- **Mutation testing (TRA-094 fixed in round 5; TRA-D5-011):** `mutmut` is now configured in `pyproject.toml` (`[tool.mutmut]` section with `paths_to_mutate = "tra"`). Added to dev deps. Run with `mutmut run`; expected mutation score ≥80%.
- **HITL testability (TRA-E5-005 fixed in round 5):** `--force-unrecoverable` debug flag added to the CLI. Injects a synthetic BLOCKING diagnostic that the repair loop can't resolve, forcing the UNRECOVERABLE → HITL path. Enables true e2e testing of the HITL flow without needing a genuinely unrepairable input.
...
- **Phase 7 (documentation & delivery)** COMPLETE: `TRA-MODULE-AUTHORING.md` (TRA-100, R4 Batch 2), `docs/adr/README.md` (8 ADRs, R5 Batch I), `docs/api-reference.md` (10 public modules, R5 Batch I), `docs/conformance-self-audit.md` (L1–L4 checklist with code evidence + benchmark results, R5 Batch I), `docs/spec-cross-reference.md` (§1–§9 mapped to implementation, R5 Batch I). All implementation_plan.md Phase 7 items checked off.
...
The full 41-finding Round 2 audit register is in `docs/audit/round2/TRA_audit_findings_register_r2.xlsx`; the narrative report is `docs/audit/round2/TRA_Prototype_Audit_Report_r2.docx`. Round 1 deliverables are in `docs/audit/` (top level). Round 3 deliverables (36 findings: 2 BLOCKING both fixed, 18 WARNING, 16 INFO) are in `docs/audit/round3/`. Round 4 deliverables (47 issues + 19 positive verifications: 1 BLOCKING, 11 WARNING, 35 INFO) are in `docs/audit/round4/`. Round 5 deliverables (68 findings: 46 issues + 22 positive verifications; 0 BLOCKING / 7 WARNING / 39 INFO) are in `docs/audit/round5/`. Round 6 deliverables (76 findings: 58 issues + 18 positive verifications; 4 BLOCKING / 10 WARNING / 44 INFO) are in `docs/audit/round6/`. Round 7 deliverables (75 findings: 39 issues + 36 positive verifications; 4 BLOCKING all R6-fixed / 18 WARNING / 53 INFO) are in `docs/audit/round7/`. The current test count is **332 across 16 test files** (was 309 at Round 6 audit baseline; +23 from Round 7 remediation commits covering TRA-B7-002 Authorization regex, TRA-B7-001 mutmut config rename, TRA-A7-004 EntityAmbiguity EXCEPTION_HANDLER, TRA-A7-001 cache-hit re-emission, TRA-A7-002 segment_index plumbing, TRA-D7-001 hardcoded paths, TRA-B7-003 pickle else removal, TRA-A7-003 leaf dedup, TRA-A7-005 ISA wrap, TRA-D7-005/006/007/008 test hardening, TRA-D7-008 /tmp path isolation).
