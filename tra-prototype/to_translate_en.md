...
It handles Chinese-English structural differences (e.g., parataxis to hypotaxis) and epistemic-modal mappings through language modules (such as TRA-MODULE-ZH-EN.md). The canonical term Confirmed is mapped to Confirmed with exact epistemic certainty, never weakened or strengthened. Meanwhile, the policy engine arbitrates conflicts through a priority stack — for example, Factual Integrity has higher priority than Target Fluency, ensuring accuracy takes precedence over eloquence.
