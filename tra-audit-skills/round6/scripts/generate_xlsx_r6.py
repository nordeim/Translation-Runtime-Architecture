"""Generate the Round 6 audit findings register XLSX.

Multi-sheet workbook mirroring the R4 register shape:
  - Summary
  - Findings (full register, autofilter)
  - Per-track sheets (A6/B6/C6/D6/E6/F6)
  - R5 Status (carry-over matrix from R4 master register)
  - Remediation Backlog (priority-sorted with effort estimates)

Output: /home/z/my-project/Translation-Runtime-Architecture/docs/audit/round6/TRA_audit_findings_register_r6.xlsx
"""
...
OUT = Path(
    "/home/z/my-project/Translation-Runtime-Architecture/docs/audit/round6/TRA_audit_findings_register_r6.xlsx"
)
...
def build_summary_sheet(wb: Workbook, findings: list[dict]) -> None:
    ...
    rows = [
        ["TRA Round 6 Audit — Summary", ""],
        ["HEAD audited", "c4ecd41"],
        ["Audit date", "2026-07-19"],
        ["Methodology", "7-track parallel re-audit (R6/A6/B6/C6/D6/E6/F6)"],
    ...
    for t in ["A6", "B6", "C6", "D6", "E6", "F6"]:
        rows.append([f"  {t}", by_track.get(t, 0)])
    rows.append(["", ""])
    rows.append(
        [
            "Track R6 (regression baseline)",
            "66 R4 entries (47 issues + 19 positives) re-verified: see R5 Status sheet",
        ]
    )
    rows.append(
        [
            "Round 4 BLOCKING findings",
            "TRA-C4-013 (tra-prototype/README.md CLI examples) — verify fixed in R6",
        ]
    )
    rows.append(["", ""])
    rows.append(
        ["4 critical invariants", "Status verified at HEAD c4ecd41 — see Track A6"]
    )
    rows.append(
        [
...
def build_r4_status_sheet(wb: Workbook, findings: list[dict]) -> None:
    ...
    Reads the R4 master register and lists every R4 finding with its R6
    re-verification status, which is sourced from the R6 master register's
    `round6_status` field. R4 findings without an R6 counterpart (e.g.,
    fixed-and-not-carried-forward) are listed with status "fixed-and-verified".
    """
    ws = wb.create_sheet("R5 Status")
    # Load R4 master register
    r4_findings: list[dict] = []
    if R5_REGISTER.exists():
        r4_findings = json.loads(R5_REGISTER.read_text(encoding="utf-8"))

    # Build a lookup: r4_id -> r6_status (from R6 master)
    r6_lookup: dict[str, str] = {}
    for f in findings:
        # The R6 finding may cite its R4 root in round6_status text or root_id
        rid = f.get("root_id", "")
        if rid.startswith("TRA-"):
            r6_lookup[rid] = f.get("round6_status", "new")
        # Also lookup by exact ID match if R6 used same root_id as R4
        r6_lookup[f["id"]] = f.get("round6_status", "new")

    ws.append(
        ["R4 ID", "R4 Severity", "R4 Title (truncated)", "R4 Finding Type", "R6 Status"]
    )
    style_header(ws, 5)
    for r4 in r4_findings:
        ...
        r6_status = r6_lookup.get(r4_id, "fixed-and-verified (not carried forward)")
        ws.append([r4_id, r4_sev, r4_title, r4_ftype, r6_status])
        if r4_sev in SEVERITY_FILL:
            ws.cell(row=ws.max_row, column=2).fill = SEVERITY_FILL[r4_sev]
        status_lower = r6_status.lower()
...
def main() -> None:
    ...
    for track in ["A6", "B6", "C6", "D6", "E6", "F6"]:
