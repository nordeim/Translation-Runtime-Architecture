...
def main() -> None:
    ...
    for f in master:
        ...
        # Normalize the round6_status to a single token
        ...
        if "fixed" in status_lower and "verified" in status_lower:
            ...
            key = "other"
        by_round6_status[key] = by_round6_status.get(key, 0) + 1
    ...
    summary = {
        ...
        "by_round6_status_normalized": dict(
            sorted(by_round6_status.items(), key=lambda x: -x[1])
        ),
    }
    ...
    with txt_path.open("w", encoding="utf-8") as fh:
        ...
        for s, n in summary["by_round6_status_normalized"].items():
            fh.write(f"  {s:25s}: {n}\n")
