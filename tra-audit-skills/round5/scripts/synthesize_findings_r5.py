"""Round 5 audit — synthesize the master findings register.

Reads the 7 per-track findings files (track_{r5,a5,b5,c5,d5,e5,f5}_findings.md),
deduplicates findings that cite the same root issue across tracks, and writes:

  - master_findings_register_r5.json   (machine-readable single source of truth)
  - summary.json / summary.txt          (counts by severity / track / category)

Output dir: /home/z/my-project/Translation-Runtime-Architecture/docs/audit/round5/
"""
...
ROUND5_DIR = Path(
    "/home/z/my-project/Translation-Runtime-Architecture/docs/audit/round5"
)
TRACKS = ["r5", "a5", "b5", "c5", "d5", "e5", "f5"]
...
def parse_findings(track: str, md_text: str) -> list[dict]:
    ...
    for m in pattern.finditer(md_text):
        ...
        body = m.group(2)
        # Extract severity, category, evidence, detail, suggested_fix, round4_status
        ...
        round4_status = _extract_field(body, "Round 4 status") or ""
...
def build_summary(master: list[dict]) -> dict:
    ...
    for f in master:
        ...
        status = f["round4_status"] or "new"
        by_round4_status[status] = by_round4_status.get(status, 0) + 1
    ...
    return {
        ...
        "by_round4_status": by_round4_status,
...
def main() -> None:
    ...
    for track in TRACKS:
        path = ROUND5_DIR / f"track_{track}_findings.md"
        if not path.exists():
    ...
    print(f"  By Round 4 status: {summary['by_round4_status']}")
    print(f"  By finding type: {summary['by_finding_type']}")

    # Write master register
    master_path = ROUND5_DIR / "master_findings_register_r5.json"
    ...
    # Write summary
    summary_path = ROUND5_DIR / "summary.json"
    ...
    txt_path = ROUND5_DIR / "summary.txt"
    with txt_path.open("w", encoding="utf-8") as fh:
        fh.write("TRA Round 5 Audit — Master Findings Register Summary\n")
        fh.write("=" * 60 + "\n\n")
        fh.write(f"HEAD audited: 5476faf\n")
        ...
        for t, n in sorted(summary["by_track"].items()):
            fh.write(f"  {t}: {n}\n")
        fh.write("\nBy Round 4 status:\n")
        for s, n in sorted(summary["by_round4_status"].items()):
            fh.write(f"  {s}: {n}\n")
        fh.write("\nBy category (top 10):\n")
