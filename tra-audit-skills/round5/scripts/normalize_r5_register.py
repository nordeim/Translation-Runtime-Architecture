...
def main() -> None:
    ...
    for f in master:
        status = f.get("round4_status") or "new"
        # Normalize the round4_status to a single token
        ...
        if "fixed" in status_lower and "verified" in status_lower:
            ...
            key = "new"
        elif "new-regression" in status_lower:
            key = "new-regression"
        elif "verified" in status_lower or "safe" in status_lower or "holding" in status_lower:
            key = "verified-holding"
        elif "carry" in status_lower:
            key = "carry-over"
        else:
            key = "other"
        by_round4_status[key] = by_round4_status.get(key, 0) + 1
    ...
    summary = {
        ...
        "by_round4_status_normalized": dict(
            sorted(by_round4_status.items(), key=lambda x: -x[1])
        ),
    }
    ...
    with txt_path.open("w", encoding="utf-8") as fh:
        ...
        for t, n in summary["by_track"].items():
            fh.write(f"  {t}: {n}\n")
        fh.write("\nBy Round 4 status (normalized):\n")
        for s, n in summary["by_round4_status_normalized"].items():
            fh.write(f"  {s:25s}: {n}\n")
